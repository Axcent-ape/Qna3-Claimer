    ...
    while True:
        ...
        if await retry_function(qna.login, thread):
            ...
            if points >= config.MINIMUM_POINTS:
                ...
                if status:
                    logger.success(f"Поток {thread} | Заклеймил {points} поинта(ов)! {qna.web3_utils.acct.address}:{tx_hash}")
                    await add_line('data/checked_accounts.txt', f"{qna.web3_utils.acct.key.hex()}:{points}")
                else:
                    logger.error(f"Поток {thread} | Не заклеймил {points} поинта(ов)! {qna.web3_utils.acct.address}:{tx_hash}")
            else:
                logger.warning(f"Поток {thread} | На аккаунте {qna.web3_utils.acct.address} мешьше {config.MINIMUM_POINTS} поинта(ов); Баланс: {points}")
                await add_line('data/checked_accounts.txt', f"{qna.web3_utils.acct.key.hex()}:{points}")
...
async def sleep(thread):
    rt = random.randint(config.DELAY[0], config.DELAY[1])
